...
def run(harness, log):
    ...
    with open(log, "w") as output:
        ...
        try:
            ...
            while process.poll() is None:
                for pid in descendants(process.pid):
                    try:
                        ...
                        if role and role not in tracked:
                            tracked[role] = os.pidfd_open(pid)
            ...
            if not expected.issubset(tracked):
                raise RuntimeError(f"Native fixture did not observe: {expected - tracked.keys()}")
            for role, fd in tracked.items():
